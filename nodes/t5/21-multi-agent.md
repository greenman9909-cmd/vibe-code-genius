...
Tier: 5  Prereqs: [24]  Parallel with: [22, 23]  Input: session + artifacts  Output: multi-agent-report.json  Model: opus  Budget: 3000 tokens
...
2. Inspect the input and extract only facts relevant to multi-agent.
3. Produce `multi-agent-report.json` with deterministic ordering, explicit nulls, and no invented evidence.
...
The output is `multi-agent-report.json`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py --node 21 --artifact multi-agent-report.json --schema schema/multi-agent-report.schema.json. If validation fails, halt. Do not substitute a different artifact. Do not continue.
...
```json
{
  "node": "21",
  "status": "complete",
  "artifact": "multi-agent-report.json",
  "version": "1.0.0",
