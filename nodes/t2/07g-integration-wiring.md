...
Tier: 2  Prereqs: [07b, 07e]  Parallel with: [19a, 20c]  Input: integrations + server  Output: webhooks/integration-events.ts  Model: sonnet  Budget: 4000 tokens
...
The output is `webhooks/integration-events.ts`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py --node 07g --artifact webhooks/integration-events.ts --schema schema/webhook-handler.schema.json. If validation fails, halt. Do not substitute a different artifact. Do not continue.
