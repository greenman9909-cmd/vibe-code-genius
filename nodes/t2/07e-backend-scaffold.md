...
2. Inspect the input and extract only facts relevant to backend scaffold.
3. Produce `server + routes` with deterministic ordering, explicit nulls, and no invented evidence.
...
Before marking complete: python scripts/validate-artifact.py server + routes schema/node.schema.json. If validation fails, halt. Do not mark complete.
