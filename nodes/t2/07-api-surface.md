...
2. Inspect the input and extract only facts relevant to api surface.
3. Produce `api_client + types` with deterministic ordering, explicit nulls, and no invented evidence.
...
Before marking complete: python scripts/validate-artifact.py api_client + types schema/node.schema.json. If validation fails, halt. Do not mark complete.
