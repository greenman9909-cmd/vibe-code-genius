...
Tier: 2  Prereqs: [02]  Parallel with: [11a, 11b, 11c, 11d, 12]  Input: reference.json.design_tokens  Output: design-tokens.json  Model: sonnet  Budget: 3500 tokens
...
2. Inspect the input and extract only facts relevant to design tokens.
3. Produce `design-tokens.json` with deterministic ordering, explicit nulls, and no invented evidence.
...
The output is `design-tokens.json`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py --node 11 --artifact design-tokens.json --schema schema/tokens.schema.json. If validation fails, halt. Do not substitute a different artifact. Do not continue.
