...
Tier: 2  Prereqs: [01d]  Parallel with: [07c, 07g, 17a]  Input: system.json.integrations  Output: integrations/research.md  Model: sonnet  Budget: 3500 tokens
...
The output is `integrations/research.md`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py --node 07b --artifact integrations/research.md --schema schema/integrations.schema.json. If validation fails, halt. Do not substitute a different artifact. Do not continue.
