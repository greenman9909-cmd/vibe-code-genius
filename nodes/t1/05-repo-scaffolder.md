...
2. Inspect the input and extract only facts relevant to repo scaffolder.
3. Produce `scaffold/` with deterministic ordering, explicit nulls, and no invented evidence.
...
Before marking complete: python scripts/validate-artifact.py scaffold/ schema/node.schema.json. If validation fails, halt. Do not mark complete.
