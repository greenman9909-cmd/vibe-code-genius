...
Tier: 1  Prereqs: [06]  Parallel with: [none]  Input: prompt.md  Output: design-commitment.md  Model: haiku  Budget: 800 tokens
...
2. Inspect the input and extract only facts relevant to design commitment.
3. Produce `design-commitment.md` with deterministic ordering, explicit nulls, and no invented evidence.
...
The output is `design-commitment.md`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py design-commitment.md schema/design-commitment.schema.json. If validation fails, halt. Do not mark complete.
