# Node 01a — Suitability Check

Tier: 1  Prereqs: [01]  Parallel with: [01b]  Input: intent.json  Output: suitability.json  Model: haiku  Budget: 800 tokens
...
2. Inspect the input and extract only facts relevant to suitability check.
3. Produce `suitability.json` with deterministic ordering, explicit nulls, and no invented evidence.
...
The output is `suitability.json`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py --node 01a --artifact suitability.json --schema schema/suitability.schema.json. If validation fails, halt. Do not substitute a different artifact. Do not continue.
