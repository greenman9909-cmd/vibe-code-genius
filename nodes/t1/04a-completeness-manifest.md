# Node 04a — Completeness Manifest

Tier: 1  Prereqs: [04]  Parallel with: [12, 13, 24]  Input: file-tree.md  Output: manifest.json  Model: sonnet  Budget: 2000 tokens
...
2. Inspect the input and extract only facts relevant to completeness manifest.
3. Produce `manifest.json` with deterministic ordering, explicit nulls, and no invented evidence.
...
The output is `manifest.json`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py --node 04a --artifact manifest.json --schema schema/manifest.schema.json. If validation fails, halt. Do not substitute a different artifact. Do not continue.
