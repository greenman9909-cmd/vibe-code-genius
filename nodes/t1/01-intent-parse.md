# Node 01 — Intent Parse

Tier: 1  Prereqs: [none]  Parallel with: [01a, 01b, 01c, 01d]  Input: free text  Output: intent.json  Model: haiku  Budget: 1500 tokens
...
2. Inspect the input and extract only facts relevant to intent parse.
3. Produce `intent.json` with deterministic ordering, explicit nulls, and no invented evidence.
...
The output is `intent.json`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py --node 01 --artifact intent.json --schema schema/intent.schema.json. If validation fails, halt. Do not substitute a different artifact. Do not continue.
