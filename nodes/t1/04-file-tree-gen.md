# Node 04 — File Tree Gen

Tier: 1  Prereqs: [03]  Parallel with: [04a, 05]  Input: style-rules.md  Output: file-tree.md  Model: sonnet  Budget: 2500 tokens
...
2. Inspect the input and extract only facts relevant to file tree gen.
3. Produce `file-tree.md` with deterministic ordering, explicit nulls, and no invented evidence.
...
The output is `file-tree.md`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py --node 04 --artifact file-tree.md --schema schema/file-tree.schema.json. If validation fails, halt. Do not substitute a different artifact. Do not continue.
