...
Tier: 1  Prereqs: [02, 01d]  Parallel with: [04, 06]  Input: reference.json + system.json  Output: style-rules.md  Model: haiku  Budget: 800 tokens
...
2. Inspect the input and extract only facts relevant to code style lock.
3. Produce `style-rules.md` with deterministic ordering, explicit nulls, and no invented evidence.
...
The output is `style-rules.md`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py --node 03 --artifact style-rules.md --schema schema/style-rules.schema.json. If validation fails, halt. Do not substitute a different artifact. Do not continue.
