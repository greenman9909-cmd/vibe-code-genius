...
2. Inspect the input and extract only facts relevant to data fetching.
3. Produce `data-hooks/` with deterministic ordering, explicit nulls, and no invented evidence.
...
Before marking complete: python scripts/validate-artifact.py data-hooks/ schema/node.schema.json. If validation fails, halt. Do not mark complete.
