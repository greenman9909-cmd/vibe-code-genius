# Node 12a — Motion Layer

Tier: 3  Prereqs: [12]  Parallel with: [12b, 13]  Input: component kit  Output: motion-layer.md  Model: haiku  Budget: 1500 tokens
...
2. Inspect the input and extract only facts relevant to motion layer.
3. Produce `motion-layer.md` with deterministic ordering, explicit nulls, and no invented evidence.
...
The output is `motion-layer.md`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py --node 12a --artifact motion-layer.md --schema schema/motion-layer.schema.json. If validation fails, halt. Do not substitute a different artifact. Do not continue.
