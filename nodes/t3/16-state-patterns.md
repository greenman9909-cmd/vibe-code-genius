...
2. Inspect the input and extract only facts relevant to state patterns.
3. Produce `state-model.md` with deterministic ordering, explicit nulls, and no invented evidence.
...
The output is `state-model.md`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py --node 16 --artifact state-model.md --schema schema/state-model.schema.json. If validation fails, halt. Do not substitute a different artifact. Do not continue.
