...
2. Inspect the input and extract only facts relevant to form logic.
3. Produce `form-flows/` with deterministic ordering, explicit nulls, and no invented evidence.
...
Before marking complete: python scripts/validate-artifact.py form-flows/ schema/node.schema.json. If validation fails, halt. Do not mark complete.
