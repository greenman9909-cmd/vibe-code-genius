...
2. Inspect the input and extract only facts relevant to schema deep infer.
3. Produce `schema-deep.json` with deterministic ordering, explicit nulls, and no invented evidence.
...
The output is `schema-deep.json`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py --node 17a --artifact schema-deep.json --schema schema/schema-deep.schema.json. If validation fails, halt. Do not substitute a different artifact. Do not continue.
