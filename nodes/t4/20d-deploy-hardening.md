# Node 20d — Deploy Hardening

Tier: 4  Prereqs: [20, 18d]  Parallel with: [23b]  Input: deployment + security  Output: deploy-hardening.md  Model: sonnet  Budget: 2200 tokens
...
2. Inspect the input and extract only facts relevant to deploy hardening.
3. Produce `deploy-hardening.md` with deterministic ordering, explicit nulls, and no invented evidence.
...
The output is `deploy-hardening.md`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py --node 20d --artifact deploy-hardening.md --schema schema/deploy-hardening.schema.json. If validation fails, halt. Do not substitute a different artifact. Do not continue.
