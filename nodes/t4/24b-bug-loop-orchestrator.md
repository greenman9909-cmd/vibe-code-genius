...
2. Inspect the input and extract only facts relevant to bug loop orchestrator.
3. Produce `wiring_report.json + security_report.json` with deterministic ordering, explicit nulls, and no invented evidence.
...
Before marking complete: python scripts/validate-artifact.py wiring_report.json + security_report.json schema/node.schema.json. If validation fails, halt. Do not mark complete.
...
```json
{
  "node": "24b",
  "status": "complete",
  "artifact": "wiring_report.json + security_report.json",
  "version": "1.0.0",
