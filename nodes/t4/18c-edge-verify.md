# Node 18c — Edge Verify

Tier: 4  Prereqs: [16a]  Parallel with: [18d, 24b]  Input: runtime report  Output: edge-report.json  Model: sonnet  Budget: 2500 tokens
...
2. Inspect the input and extract only facts relevant to edge verify.
3. Produce `edge-report.json` with deterministic ordering, explicit nulls, and no invented evidence.
...
The output is `edge-report.json`. It is versioned, machine-readable when the artifact is JSON, and contains a `version`, `generated_at`, `evidence`, and `status` field where the artifact shape permits.

Before marking complete: python scripts/validate-artifact.py --node 18c --artifact edge-report.json --schema schema/edge-report.schema.json. If validation fails, halt. Do not substitute a different artifact. Do not continue.
